"""
Experiment: Bidirectional LNN residual imputation with trajectory-aware loss.

Compared models:
1. Bidirectional_LNN
2. BiLSTM
3. Hybrid_BiLNN_BiLSTM

Main additions over the baseline script:
- weak observed reconstruction loss
- short-window trajectory-aware loss on acceleration-derived displacement
- multi missing-rate evaluation and checkpoint reuse
"""
...
class TrajectoryAwareReconstructionLoss(nn.Module):
    def __init__(self, w_missing: float = 1.0, w_observed: float = 0.02, w_traj: float = 0.1, traj_window: int = 5):
        super().__init__()
    ...
    def forward(self, pred, target, mask, uncertainty=None, dt=None):
        missing = 1.0 - mask
        mse_missing = ((pred - target) ** 2 * missing).sum() / (missing.sum() + 1e-8)
        mse_observed = ((pred - target) ** 2 * mask).sum() / (mask.sum() + 1e-8)

        if pred.shape[1] > self.traj_window:
            acc_pred = pred[:, :, 3:6]
            acc_target = target[:, :, 3:6]
            dt_seq = dt.squeeze(-1) if dt is not None and dt.dim() == 3 else dt
            if dt_seq is None:
                dt_seq = torch.ones(pred.shape[0], pred.shape[1], device=pred.device, dtype=pred.dtype)
            acc_step_pred = acc_pred[:, 1:] * dt_seq[:, 1:].unsqueeze(-1)
            acc_step_target = acc_target[:, 1:] * dt_seq[:, 1:].unsqueeze(-1)
            pred_disp = torch.cumsum(acc_step_pred, dim=1)
            target_disp = torch.cumsum(acc_step_target, dim=1)
            pred_rel = pred_disp[:, self.traj_window - 1:] - pred_disp[:, :-self.traj_window + 1]
            target_rel = target_disp[:, self.traj_window - 1:] - target_disp[:, :-self.traj_window + 1]
            traj_loss = F.mse_loss(pred_rel, target_rel)
        else:
            traj_loss = torch.tensor(0.0, device=pred.device)

        total = self.w_missing * mse_missing + self.w_observed * mse_observed + self.w_traj * traj_loss
        return total, {
            "recon": float(mse_missing.item()),
            "observed": float(mse_observed.item()),
...
class ResidualInputAdapter(nn.Module):
    ...
    def build(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        left_dist_list, right_dist_list, rel_pos_list, missing_indicator_list = [], [], [], []
        ...
        for b in range(x.shape[0]):
            ...
            missing_indicator_list.append(1.0 - obs_mask)
            rel_pos_list.append(torch.linspace(0.0, 1.0, x.shape[1], device=x.device, dtype=x.dtype))
        ...
        enriched = torch.cat([
            residual_observation, mask, dt, delta, window_feats, baseline,
            left_dist, right_dist, rel_pos, missing_indicator,
        ], dim=-1)
...
class BidirectionalLNNResidual(nn.Module):
    def __init__(self, input_dim: int, feature_dim: int, hidden_units: int = 128, output_dim: int = 6):
        ...
        self.head = nn.Sequential(nn.Linear(hidden_units * 2, hidden_units), nn.ReLU(), nn.Linear(hidden_units, output_dim))
        self.uncertainty_head = nn.Sequential(nn.Linear(hidden_units * 2, hidden_units // 2), nn.ReLU(), nn.Linear(hidden_units // 2, output_dim), nn.Softplus())

    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        enriched, baseline = self.adapter.build(x)
        fwd_h, _ = self.forward_lnn(enriched)
        bwd_h_rev, _ = self.backward_lnn(torch.flip(enriched, dims=[1]))
        h = torch.cat([fwd_h, torch.flip(bwd_h_rev, dims=[1])], dim=-1)
        residual = self.head(h)
        return baseline + residual, self.uncertainty_head(h)
...
class ResidualBiLSTM(nn.Module):
    def __init__(self, input_dim: int, feature_dim: int, hidden_dim: int = 128, output_dim: int = 6, num_layers: int = 2):
        ...
        self.adapter = ResidualInputAdapter(feature_dim=feature_dim, input_dim=input_dim)
        residual_input_dim = input_dim + feature_dim + 4
        self.backbone = LongTermLSTM(input_dim=residual_input_dim, hidden_dim=hidden_dim, output_dim=output_dim, num_layers=num_layers)

    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        enriched, baseline = self.adapter.build(x)
        residual, uncertainty = self.backbone(enriched)
        return baseline + residual, uncertainty
...
class ResidualHybridBiLNNBiLSTM(nn.Module):
    def __init__(self, input_dim: int, feature_dim: int, lnn_hidden: int = 128, lstm_hidden: int = 64, output_dim: int = 6, lstm_layers: int = 2):
        super().__init__()
        self.bi_lnn = BidirectionalLNNResidual(input_dim=input_dim, feature_dim=feature_dim, hidden_units=lnn_hidden, output_dim=output_dim)
        self.bi_lstm = ResidualBiLSTM(input_dim=input_dim, feature_dim=feature_dim, hidden_dim=lstm_hidden, output_dim=output_dim, num_layers=lstm_layers)
        gate_input_dim = output_dim * 4 + input_dim
        self.gate_net = nn.Sequential(nn.Linear(gate_input_dim, 32), nn.ReLU(), nn.Linear(32, output_dim), nn.Sigmoid())
        self.combined_uncertainty = nn.Sequential(nn.Linear(output_dim * 2, output_dim), nn.Softplus())

    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        lnn_pred, lnn_unc = self.bi_lnn(x)
        lstm_pred, lstm_unc = self.bi_lstm(x)
        gate = self.gate_net(torch.cat([lnn_pred, lstm_pred, lnn_unc, lstm_unc, x], dim=-1))
...
def _evaluate_across_missing_rates(model, config: dict, device: torch.device, criterion: nn.Module, missing_rates: List[float]) -> List[dict]:
    ...
    for rate in missing_rates:
        test_ds = CfCIMUDataset(
            root_dir=config["root_dir"], seq_len=config["seq_len"], mask_rate=rate, missing_mode=config["missing_mode"],
        ...
        test_loader = torch.utils.data.DataLoader(
            test_ds, batch_size=config["batch_size"], shuffle=False, num_workers=config["num_workers"], pin_memory=device.type == "cuda"
        )
...
def _plot_summary(summary_df: pd.DataFrame, output_path: Path, timestamp: str):
    ...
    plt.tight_layout()
    plt.savefig(output_path / f"comparison_bidirectional_lnn_traj_{timestamp}.png", dpi=300, bbox_inches="tight")
    plt.close()
...
def run_experiment(
    ...
    output_dir: str = "results/bidirectional_lnn_residual_trajectory",
):
    config = {
    ...
    train_ds = CfCIMUDataset(root_dir=config["root_dir"], seq_len=config["seq_len"], mask_rate=config["mask_rate"], missing_mode=config["missing_mode"], split="train", split_ratio=config["train_ratio"], val_ratio=config["val_ratio"], eval_mode=False, include_window_features=config["include_window_features"])
    val_ds = CfCIMUDataset(root_dir=config["root_dir"], seq_len=config["seq_len"], mask_rate=config["mask_rate"], missing_mode=config["missing_mode"], split="val", split_ratio=config["train_ratio"], val_ratio=config["val_ratio"], eval_mode=True, include_window_features=config["include_window_features"])
    test_ds = CfCIMUDataset(root_dir=config["root_dir"], seq_len=config["seq_len"], mask_rate=config["mask_rate"], missing_mode=config["missing_mode"], split="test", split_ratio=config["train_ratio"], val_ratio=config["val_ratio"], eval_mode=True, return_stats=True, return_vicon=True, include_window_features=config["include_window_features"])
    ...
    criterion = TrajectoryAwareReconstructionLoss()
    models_to_train = {
        "Bidirectional_LNN": BidirectionalLNNResidual(input_dim=config["input_dim"], feature_dim=config["feature_dim"], hidden_units=config["lnn_hidden"], output_dim=6),
        "BiLSTM": ResidualBiLSTM(input_dim=config["input_dim"], feature_dim=config["feature_dim"], hidden_dim=config["lstm_hidden"], output_dim=6, num_layers=config["lstm_layers"]),
        "Hybrid_BiLNN_BiLSTM": ResidualHybridBiLNNBiLSTM(input_dim=config["input_dim"], feature_dim=config["feature_dim"], lnn_hidden=config["lnn_hidden"], lstm_hidden=config["lstm_hidden"], output_dim=6, lstm_layers=config["lstm_layers"]),
    }
    ...
    summary_csv = output_path / f"summary_bidirectional_lnn_traj_{timestamp}.csv"
    history_csv = output_path / f"history_bidirectional_lnn_traj_{timestamp}.csv"
    multi_rate_csv = output_path / f"missing_rate_comparison_{timestamp}.csv"
    excel_path = output_path / f"bidirectional_lnn_residual_trajectory_{timestamp}.xlsx"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Bidirectional residual experiment with trajectory-aware loss")
    ...
    parser.add_argument("--output_dir", type=str, default="results/bidirectional_lnn_residual_trajectory")

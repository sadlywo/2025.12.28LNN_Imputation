"""Experiment: seq_len ablation for GRU/Transformer under reconstruction-only loss."""
...
def run_seq_len_ablation(
    ...
    output_dir: str = "results/seq_len_ablation_gru_transformer_recon_only",
):
    config = {
    ...
    print("=" * 80)
    print("SEQ_LEN ABLATION (GRU/TRANSFORMER, RECONSTRUCTION-ONLY)")
    print("=" * 80)
    ...
    for model_name in config["models"]:
        for seq_len in config["seq_len_values"]:
            seq_len = int(seq_len)
            exp_name = f"{model_name}_seq{seq_len}"
    ...
    df_history["series"] = df_history["model"].str.upper() + "_seq" + df_history["seq_len"].astype(str)

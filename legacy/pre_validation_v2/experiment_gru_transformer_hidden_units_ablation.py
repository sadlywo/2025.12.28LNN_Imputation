"""Experiment: GRU/Transformer hidden_units ablation under reconstruction-only loss."""
...
def run_hidden_units_ablation():
    config = {
        ...
        "num_workers": 4,
        "seed": 42,
        "output_dir": "results/gru_transformer_hidden_units_ablation",
    ...
    print("=" * 80)
    print("GRU/TRANSFORMER HIDDEN_UNITS ABLATION (RECONSTRUCTION-ONLY)")
    print("=" * 80)
    ...
    for model_name in config["models"]:
        for hidden_units in config["hidden_units_list"]:
            exp_name = f"{model_name}_h{hidden_units}"
            print(f"\n{'=' * 80}")
    ...
    df_summary = pd.DataFrame(summary_rows).sort_values(
        by=["model", "best_val_mse_masked", "best_val_loss"],
        ascending=[True, True, True],
    )
    df_history = pd.DataFrame(history_rows)
    df_weights = pd.DataFrame(weight_rows).sort_values(by=["model", "hidden_units"])
    ...
    excel_path = output_path / f"gru_transformer_hidden_units_ablation_{timestamp}.xlsx"
    try:
        with pd.ExcelWriter(excel_path, engine="openpyxl") as writer:
            ...
            pd.DataFrame([{"key": k, "value": v} for k, v in config.items()]).to_excel(
                writer, index=False, sheet_name="config"
            )
    ...
    try:
        ...
        for model_name in config["models"]:
            df_s = df_summary[df_summary["model"] == model_name].sort_values("hidden_units")
            plt.figure(figsize=(10, 5))
            plt.plot(df_s["hidden_units"], df_s["best_val_mse_masked"], marker="o", linewidth=2)
            plt.title(f"{model_name.upper()} Hidden Units Ablation (Reconstruction-Only)")
            plt.xlabel("hidden_units")
            plt.ylabel("Best Val MSE (masked)")
            plt.grid(True, alpha=0.3)
            plt.tight_layout()
            plot_path = output_path / f"mse_vs_hidden_units_{model_name}_{timestamp}.png"
        ...
        for model_name in config["models"]:
            ...
            plt.plot(
                df_s["hidden_units"],
                df_s["best_val_mse_masked"],
                marker="o",
                linewidth=2,
                label=model_name.upper(),
            )
        plt.title("GRU vs Transformer: MSE vs Hidden Units (Reconstruction-Only)")
        plt.xlabel("hidden_units")
        plt.ylabel("Best Val MSE (masked)")
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plot_path = output_path / f"mse_vs_hidden_units_compare_{timestamp}.png"
        plt.savefig(plot_path, dpi=300, bbox_inches="tight")
        plt.close()
    except Exception as e:

"""Experiment: Stabilized physics loss weight ablation (denorm + normalized physics)."""
import torch
import numpy as np
import pandas as pd
...
def experiment_physics_weights_ablation_stable():
    config = {
        ...
        "output_dir": "results/physics_loss_weights_ablation_stable",
        "num_workers": 4,
        "drift_scale": 0.01,
    ...
    print("=" * 80)
    print("PHYSICS LOSS WEIGHT ABLATION (DENORM + NORMALIZED PHYSICS)")
    print("=" * 80)
    ...
    w_integration_values = [0.0, 1e-4, 5e-4, 1e-3, 5e-3]
    w_energy_values = [0.0, 1e-4, 5e-4, 1e-3, 5e-3]

    results = {}
    for w_integration in w_integration_values:
        for w_energy in w_energy_values:
            exp_name = f"pi_stable_wi{w_integration:g}_we{w_energy:g}"
    ...
    print("\n" + "=" * 80)
    print("DONE: PHYSICS LOSS WEIGHT ABLATION (STABLE)")
...
def generate_report(results: dict, output_path: Path, timestamp: str, config: dict):
    ...
    summary_path = output_path / f"summary_physics_loss_weights_stable_{timestamp}.csv"
    df_summary.to_csv(summary_path, index=False)
    print(f"\n[Saved] Summary CSV: {summary_path}")

    mse_path = output_path / f"val_mse_masked_by_epoch_stable_{timestamp}.csv"
...
if __name__ == "__main__":
    experiment_physics_weights_ablation_stable()
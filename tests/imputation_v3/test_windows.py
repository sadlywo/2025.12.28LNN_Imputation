...
def test_canonical_zero_rate_has_one_semantic_identity():
    positive_zero = _materialize(
        [_recording(rows=16)],
        stride=16,
        exhaustive=True,
        topologies=("point",),
    ...
    assert positive_zero.window_id == negative_zero.window_id
    assert torch.equal(positive_zero.mask, negative_zero.mask)
...
def test_mask_metadata_observed_values_and_baseline_completion_are_consistent():
    ...
    assert {window.topology for window in windows} == set(generator_names)
    for window in windows:
...
def test_materialization_rejects_malformed_scaler_output(output):
    class MalformedScaler:

...
> **迁移门禁：** `configs/validation_v2/server_full.yaml` 目前仍是旧的 OxIOD
> 单数据集正式矩阵。下面的 runner 与双卡运维逻辑已经验证，但该配置尚未满足
> OxIOD、EuRoC MAV、IDOL 三数据集联合训练协议。在联合采样、联合划分和主表配置
> 完成前，不要租用付费算力把它当作最终三数据集论文实验；它只能用于 runner 诊断。

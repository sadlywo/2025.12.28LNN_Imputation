...
- `experiment_bidirectional_lnn_residual.py` is a historical residual hybrid. It uses forward/backward CfC and BiLSTM branches around an interpolation baseline, but it belongs to an older experiment path that previously included target-derived window features and did not establish valid CfC timespan semantics or gate attribution.

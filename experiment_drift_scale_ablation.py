...
def run_drift_scale_ablation(
    ...
    config = {
        ...
        "models": models or ["lnn", "gru", "transformer"],
        "drift_scale_values": drift_scale_values or [0.0, 0.01, 0.05, 0.1, 0.2],
        "w_recon": 1.0,
    }
    ...
    print("Loss: ReconstructionOnlyLoss")
    ...
    for model_name in config["models"]:
        for drift_scale in config["drift_scale_values"]:
            ...
            exp_name = f"{model_name}_drift{drift_scale:g}"
    ...
    df_summary = pd.DataFrame(summary_rows).sort_values(
        by=["model", "best_val_mse_masked", "best_val_loss"], ascending=[True, True, True]
    )
    summary_path = output_path / f"summary_drift_scale_compare_{timestamp}.csv"
    df_summary.to_csv(summary_path, index=False)
    print(f"\n[Saved] Summary CSV: {summary_path}")

    df_history = pd.DataFrame(history_rows)
    history_path = output_path / f"history_drift_scale_compare_{timestamp}.csv"
    df_history.to_csv(history_path, index=False)
    print(f"[Saved] History CSV: {history_path}")

    pivot_path = output_path / f"val_mse_masked_by_epoch_compare_{timestamp}.csv"
    df_history = df_history.copy()
    df_history["series"] = df_history["model"].str.upper() + "_drift" + df_history["drift_scale"].astype(str)
    df_pivot = df_history.pivot(index="epoch", columns="series", values="val_mse_masked").reset_index()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--models", type=str, default="lnn,gru,transformer")
    parser.add_argument("--model", dest="model_single", type=str, default=None)
    parser.add_argument("--drift_scales", type=str, default="0,0.01,0.05,0.1,0.2")
    parser.add_argument("--output_dir", type=str, default="results/drift_scale_ablation")
    args = parser.parse_args()

    run_drift_scale_ablation(
        models=[args.model_single] if args.model_single else [m.strip() for m in args.models.split(",") if m.strip()],
